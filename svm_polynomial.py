...
Out: scalar value 
'''
def kernel(x, y, degree, bias): 
	return np.power((np.dot(x,y) + bias), degree)
...
Out: gam_matrix: (n,n) 
'''
def gam_matrix(data, degree, bias, kernel):
	nr_data = data.shape[0]
	gam_matrix = np.array([kernel(data[i], data[j], degree, bias) \
...
def optmization_cv(data, degree, kernel, gam_matrix, constantVal):
	...
	nr_data = data.shape[0] 
	P = np.outer(y,y)*gam_matrix(data, degree, bias, kernel)
...
'''
def compute_b(alfa, sup_x, sup_y, degree, bias):
	# retrieve multipliers (alpha)
	# support vector x
	# support vector y	
	intercept=0.0
	for I_X_DO in range(len(sup_vector)):
		dummy = np.sum( (alfa[j]*sup_y[j]*kernel(sup_x[j], sup_x[I_X_DO], degree, bias)) \
...
def prediction(data):
	...
	for I_X_DO in range(data.shape[0]):
		y_pred[I_X_DO]=np.sign(np.sum(( alpha[j]*sup_vector_y[j]\
						                *kernel(sup_vector[j], data[I_X_DO], degree, bias) \
...
alpha, sup_vector, sup_vector_y = optmization_cv(X, degree, kernel, gam_matrix, constant_value)


# compute intercept
b = compute_b(alpha, sup_vector, sup_vector_y, degree, bias)
...
xx, yy = np.meshgrid(np.linspace(-2.5, 2.5, 150), np.linspace(-1, 3, 150))
